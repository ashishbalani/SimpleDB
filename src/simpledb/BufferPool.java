...
public class BufferPool {
	...
	public BufferPool(int numPages) {
		...
		pageAccessed=new ArrayList<PageId>();
	}
	...
	public Page getPage(TransactionId tid, PageId pid, Permissions perm) throws TransactionAbortedException,
		...
		if(pages.size()>numPages){
			throw new DbException("Maximum page request reached");
		}
		else{
			pageAccessed.add(pid);
	...
	private synchronized void flushPage(PageId pid) throws IOException {
		...
		HeapFile hf=(HeapFile)Database.getCatalog().getDbFile(((HeapPageId)pid).getTableId());
		if(p.isDirty()!=null){
			hf.writePage(p);
	...
	private synchronized void evictPage() throws DbException {
		// some code goes here
		// not necessary for assignment1
		PageId toevictPid=pageAccessed.get(0);
	
		if(pages.get(toevictPid).isDirty()!=null){
			try{
			flushPage(toevictPid);
			}catch(IOException io){
				io.printStackTrace();
			}
		}
		int i=0;
		while(i<pageAccessed.size()){
			if(pageAccessed.get(i)==toevictPid)
				pageAccessed.remove(toevictPid);
			i++;
		}
		
		pages.remove(toevictPid);
