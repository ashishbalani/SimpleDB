...
public class HeapFile implements DbFile {
	...
	public Page readPage(PageId pid) {
		...
		try{
			...
			raf.seek(pid.pageno()*BufferPool.PAGE_SIZE);
	...
	public Page deleteTuple(TransactionId tid, Tuple t) throws DbException, TransactionAbortedException {
		// some code goes here
		// not necessary for assignment1
		HeapPage hp=(HeapPage)Database.getBufferPool().getPage(tid,t.getRecordId().getPageId(),Permissions.READ_ONLY);
		hp.deleteTuple(t);
		return Database.getBufferPool().getPage(tid,t.getRecordId().getPageId(),Permissions.READ_ONLY);
	}
	...
	public DbFileIterator iterator(TransactionId tid) {
		// some code goes here
			...
			return new HeapFileDbFileIterator(tid, this);
		}
